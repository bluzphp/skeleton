...
$(function () {
        ...
        .ajaxError(function (event, jqXHR, options, thrownError) {
            try {
                ...
                if (options.dataType === 'json' ||
                    ...
                    let $modal = modal.create($(document), jqXHR.responseText, 'width: 800px');
                    modal.show($modal);
                }
                console.log(thrownError, jqXHR.responseText);
            } catch (err) {
                console.error(err.name, err.message);
            }
        })
    ...
    function ajaxLink(event) {
        ...
        $.ajax({
            url: $this.attr('href'),
            type: $this.data('ajax') || $this.data('ajax-method') || 'post',
            data: processData($this),
            dataType: $this.data('ajax-type') || 'json',
            context: this
        });
    ...
    function imagePreview(event) {
        ...
        let $modal = modal.create($this, $img, '');
        modal.show($modal);
    }
});
