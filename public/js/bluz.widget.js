...
define(['jquery', 'bluz.storage'], function ($, storage) {
  'use strict';
  // on DOM ready state
  $(function () {
    // one handler for all widgets and all controls
    $(document).on('click', '.widget .widget-control', clickWidgetControl);

    function clickWidgetControl() {
      let $this = $(this);
      let $widget;
      let $content;
      let key;
    ...
    $('.widget').each(toggleWidgetContent);

    function toggleWidgetContent(i, el) {
      let $widget;
      let $content;
      let key;
      $widget = $(el);
